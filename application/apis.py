...
def get_twit():
    try:
        ...
        ts = tss.get_ts()
        keen = Keen()
#        tso = TweetSearchSupport().generate_tso("시부야 린", today)
...
#        for girl_name in characters:
#            for date_before in xrange(7):
#                tso = TweetSearchSupport().generate_tso(girl_name, start_date)
#                ts.search_tweets(tso)
#                amount = ts.get_amount_of_tweets()
#                keen.add_girl(start_date, girl_name, amount)
        tso = tss.generate_tso("시부야 린", today)
        for tweet in ts.search_tweets_iterable(tso):
            print tss.to_datetime(tweet['created_at']) , tweet['created_at']
